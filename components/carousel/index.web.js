...
import {
    getPosition
} from '../_utils/domUtils';

const FPS = 60;
const UPDATE_INTERVAL = 1000 / FPS;
const THRESHOLD_PERCENTAGE = 0.4;
...
class Carousel extends Component {
    ...
    handleTouchMove = (e) => {
        ...
        const deltaY = y - startPositionY;
        if (swipeX && Math.abs(deltaY) < Math.abs(deltaX)) {
            // e.stopPropagation();
            if (e.cancelable) {
        ...
        } else if (swipeY && Math.abs(deltaY) > Math.abs(deltaX)) {
            this.setState({
                swipeX: false
            })
        }
